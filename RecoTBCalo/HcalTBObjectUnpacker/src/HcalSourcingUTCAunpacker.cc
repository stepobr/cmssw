...
void HcalSourcingUTCAunpacker::unpack(const FEDRawData&  raw, const HcalElectronicsMap emap, std::unique_ptr<HcalUHTRhistogramDigiCollection>& histoDigiCollection, bool DEBUG) const {
...
//Loop over data
  ...
  //  eid.setHTR(htr_cr,htr_slot,htr_tb);
    ...
    if(DEBUG)
      std::cout << "did.null() " << did.null() << "   did.det(): " << did.det() << "   did.subdetId(): " << did.subdetId() << std::endl;
...
//     std::cout << "putting event with Det Id: " << ((HcalDetId)did) << std::endl;
